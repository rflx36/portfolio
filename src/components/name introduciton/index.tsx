...
export default function NameIntroduction() {
    ...
    const [lottieInitialize, setLottieInitialize] = useState<boolean>(false);
    ...
    useEffect(() => {
        ...
        const animInitialize = setTimeout(() => {
            setLottieInitialize(true);
        }, window.innerWidth <= 430 ? 0 : 100);


        return () => {
            clearTimeout(nameInitialize);
            clearTimeout(animInitialize);
        }
    }, [])
    ...
    // const handleDotLottieRef = (index: number, dotLottie:) => {
